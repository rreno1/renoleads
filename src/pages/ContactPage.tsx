...
export function ContactPage() {
  ...
  return <>
    ...
    <section className="support-content"><div className="container support-grid"><div data-reveal="left"><span className="section-eyebrow">Inquiry</span><h2>{property ? `About ${property.title}` : 'Tell us what you are looking for.'}</h2><p>{property ? 'This form is linked to the exact NJ125 lot you selected.' : 'You can send a general inquiry even if you have not selected a property yet.'}</p><p className="text-muted">Your inquiry is sent to NJ125 when you submit. It is not buffered in browser storage if the service is unavailable.</p></div><div className="sidebar-card" data-reveal="right"><InquiryForm property={property}/></div></div></section>
  </>;
}
