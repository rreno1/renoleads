    ...
    <div className="detail-layout">
      <div className="detail-main">
        <section className="gallery-section">
          {currentMedia ? <div className="gallery"><div className="gallery-main"><img src={currentMedia.url} alt={currentMedia.alt || currentProperty.title}/><span className="gallery-count">{currentProperty.media.length} photo{currentProperty.media.length === 1 ? '' : 's'}</span></div>{currentProperty.media.length > 1 ? <div className="gallery-thumbs">{currentProperty.media.map((media, index) => <button key={`${media.url}-${index}`} type="button" className={`gallery-thumb${index === activeImage ? ' active' : ''}`} aria-label={`View property photo ${index + 1}`} onClick={() => setActiveImage(index)}><img src={media.url} alt={media.alt || `Property photo ${index + 1}`} loading="lazy"/></button>)}</div> : null}</div> : <div className="gallery gallery-placeholder"><Icon name="area"/><strong>Property photos coming soon</strong><span>Ask for current site photos in your inquiry.</span></div>}
        </section>

        <section className="spec-section"><h2 className="spec-section-title">Property details</h2><dl className="spec-grid"><Spec label="Project" value={currentProperty.project || 'Ask for details'}/><Spec label="Phase" value={currentProperty.phase || 'Ask for details'}/><Spec label="Block" value={currentProperty.block || 'Ask for details'}/><Spec label="Lot" value={currentProperty.lotNumber || 'Ask for details'}/><Spec label="Lot area" value={currentProperty.areaSqm !== null ? `${formatNumber(currentProperty.areaSqm)} sqm` : 'Ask for details'}/><Spec label="Published price" value={formatCurrency(currentProperty.displayPrice)}/><Spec label="Availability" value="Available"/><Spec label="Location" value={location}/></dl></section>

        {currentProperty.description ? <section className="spec-section detail-copy"><h2 className="spec-section-title">About this property</h2><p>{currentProperty.description}</p></section> : null}
