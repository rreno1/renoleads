    ...
    <div className="detail-layout">
      <div className="detail-main">
        <section className="gallery-section" data-reveal="up">
          {currentMedia ? <div className="gallery"><div className="gallery-main"><img src={currentMedia.url} alt={currentMedia.alt || currentProperty.title} width={1440} height={960} loading="eager" decoding="async" fetchPriority="high"/><span className="gallery-count">{currentProperty.media.length} photo{currentProperty.media.length === 1 ? '' : 's'}</span></div>{currentProperty.media.length > 1 ? <div className="gallery-thumbs">{currentProperty.media.map((media, index) => <button key={`${media.url}-${index}`} type="button" className={`gallery-thumb${index === activeImage ? ' active' : ''}`} aria-label={`View property photo ${index + 1}`} onClick={() => setActiveImage(index)}><img src={media.url} alt={media.alt || `Property photo ${index + 1}`} width={240} height={160} loading="lazy" decoding="async" fetchPriority="low"/></button>)}</div> : null}</div> : <div className="gallery gallery-placeholder"><Icon name="area"/><strong>Property photos coming soon</strong><span>Ask for current site photos in your inquiry.</span></div>}
        </section>
